...
<body>
    <div class="page-header">
        <h1 style="text-align: center;">  <b><?php echo htmlspecialchars($_SESSION["username"]); ?></b>這裡是編輯列表</h1>
    </div>
        ...
        // output data of each row
         while($row = $result->fetch_assoc()) {
           ...
           //echo $jsrow;   
             ...
             ?>
             <form action="permission_change.php" method="post" style="display: inline;">
                <label>帳號:</label>
                <input type="text" name="first_name"  disabled="disabled" value="<?php echo $UserName; ?>">
                <label>權限:</label>
                <input type="text" name="last_name"  disabled="disabled" value="<?php echo $permission; ?>">
                <input class="btn btn-primary" type="submit" name="action1" value="編輯"/>
                <input type="hidden" name="username" value="<?php echo $UserName; ?>"/> 
             </form> 
             <form action="permission_delete.php" method="post" style="display: inline;">
