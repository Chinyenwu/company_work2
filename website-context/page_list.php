...
<body>
    <div class="page-header">
        <h1 style="text-align: center;">歡迎, <b><?php echo htmlspecialchars($_SESSION["username"]); ?></b>這裡是網站功能-頁面列表</h1>
    </div>
    ...
    if ($result->num_rows > 0) {
        // output data of each row
         while($row = $result->fetch_assoc()) {
           ...
           $id=$row["id"];
             ?>
             <form action="page_change.php" method="post" style="display: inline;">
                <label>類別:</label>
                ...
                <label>編輯人:</label>
                <input type="text" name="editer"  disabled="disabled" value="<?php echo $editer; ?>">
                <label>編輯時間:</label>
                <input type="text" name="editer_time"  disabled="disabled" value="<?php echo $editer_time; ?>">
